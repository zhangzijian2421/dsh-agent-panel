...
- **任何会话拉人都会自动变群**（第 1 档行为）：目标会话没有群目录时，`pull` 会先为它创建
  ...
  （返回 `autoGrouped: true`）。若该会话用「多 Agent 聊天群」预设创建，成员额外拥有
  `group_send` 等群工具；**否则成员没有群工具**，会按成员须知里的自适应指引改用
  `send_message` 私信汇报（成员须知同时给出「有群工具走频道 / 无群工具走私信」两条路径，
  因为宿主侧无法可靠判断会话是否群预设：`composedPreset` 与 `header.agentPreset` 实测都不可靠）
