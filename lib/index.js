...
export function apply(ctx) {
  ...
  // 归属到工作区（= 出现在侧边栏工作区下面）。归属必须在宿主侧做：只有 Workspace 实体的
  // attachSession() 能给会话开户，客户端面只有"重排已归属会话"的 insertSessionBefore。
  registerRoute(ctx, '/api/dsh-agent-panel/group-attach', ['POST'], (args) => service.attach(args), 'dsh-agent-panel: group-attach route')
