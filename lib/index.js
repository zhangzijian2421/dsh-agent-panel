...
import { installMemberDispatch } from './dispatch.js'
...
function registerModelTools(ctx, service) {
  ...
  tools.register({
    ...
    description: '把当前会话变成一个群聊：注册进群聊面板，之后用 group_pull 把 agent 拉进来（当前会话即群主会话）。'
      + '群主 preset 由插件固定为「群聊 Agent」（盘点成员能力边界 + 派活 + 汇总），不能指定。',
    parameters: {
      type: 'object',
      properties: {
    ...
    async execute(args, exec) {
      ...
      return JSON.stringify(await service.createGroup({ session_id: sessionId, name: args && args.name }), null, 2)
    }
  })
}
...
export { installMentionFilter, installMemberDispatch }
...
export function apply(ctx) {
  ...
  // 两处都拦截 sessionReferenceResolver，而本插件可能比它先挂载，所以用 inject 等它出现：
  //  - @ 菜单过滤：把"别的会话的子代理"从候选里去掉；
  //  - 群内派活：群里 @ 成员时，把这条消息真的转达给那个成员（@ 本身只是引用快照）。
  ctx.effect(() => {
    let filterDispose
    let dispatchDispose
    const install = () => {
      if (filterDispose === undefined) filterDispose = installMentionFilter(ctx)
      if (dispatchDispose === undefined) dispatchDispose = installMemberDispatch(ctx, { isGroup: (id) => service.isGroup(id) })
    }
    install()
    if (filterDispose === undefined || dispatchDispose === undefined) {
      ctx.inject(['sessionReferenceResolver'], () => { install() })
    }
    return () => {
      if (typeof filterDispose === 'function') filterDispose()
      if (typeof dispatchDispose === 'function') dispatchDispose()
    }
  }, 'dsh-agent-panel: @ menu filter + member dispatch')
}
