...
function makeService(ctx) {
  ...
  async function pull(args) {
    ...
    if (parent === undefined) {
      // A blank (never-started) session has no Agent yet. Start it through the same host
      // path the GUI uses — `sessionController.ensureSession` adopts a live agent, resumes a
      // cold one, or creates it from the session's own preset — then pull into it.
      const controller = ctx.get('sessionController')
      if (controller === undefined || typeof controller.ensureSession !== 'function') {
        return { ok: false, error: '目标会话尚未启动，且无法自动启动（sessionController 不可用）。请先在该会话里发送一条消息。' }
      }
      try {
        // Honour the preset this session already carries (staged picker choice): creating it
        // from the default instead would silently ignore what the user selected.
        let stagedPreset
        try {
          const session = sessions.get(ownerId)
          const value = session && session.header ? session.header.agentPreset : undefined
          stagedPreset = typeof value === 'string' && value.length > 0 ? value : undefined
        } catch (error) { stagedPreset = undefined }
        parent = await controller.ensureSession(ownerId, cwd, true, stagedPreset)
        autoStarted = parent !== undefined
      } catch (error) {
        return { ok: false, error: '目标会话尚未启动，自动启动失败：' + errMsg(error) + '。可先在该会话里发送一条消息再拉人。' }
      }
      try { parent = agents.get(ownerId) } catch (error) { /* keep what ensureSession returned */ }
    }
    if (parent === undefined) return { ok: false, error: '目标会话不在线（' + ownerId + '）' }
    ...
    const result = { ok: true, member_id: childId, name, preset_id: presetId, group: isGroup || autoGrouped, autoGrouped, autoStarted }
    const infos = []
    if (autoStarted) infos.push('已自动启动该会话（原本尚未启动）')
    if (autoGrouped) infos.push('已自动为该会话创建独立群聊目录（自动变群）')
    if (infos.length > 0) result.info = infos.join('；') + '。群工具（group_send / group_read / group_members）由面板提供，主持人与成员都可用。'
