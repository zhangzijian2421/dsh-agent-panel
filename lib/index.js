...
async function readOwnedRoster(fs, cwd, sessionId) {
  const resolved = await resolveGroupDir(fs, cwd, sessionId)
...
function makeService(ctx) {
  ...
  async function pull(args) {
    ...
    if (fs !== undefined) {
      ...
      // A chat-group session creates its own roster in the background on its first step;
      // re-check briefly so a pull issued right after the first message still lands in
      // group mode instead of silently degrading to a plain pull.
      for (let attempt = 0; attempt < 5 && resolved.roster === undefined; attempt += 1) {
        await new Promise((resolve) => { setTimeout(resolve, 300) })
        try { resolved = await resolveGroupDir(fs, cwd, ownerId) } catch (error) { break }
      }
    }
    const roster = resolved.roster
    ...
    if (isGroup && fs !== undefined) {
      try {
        ...
        const chat = await nextChatSeq(fs, groupDir)
  ...
  async function initGroup(args) {
    ...
    try {
      ...
      const roster = { owner: { id: ownerId, name: ownerName }, members: [] }
      // fs.writeText creates missing parent directories, so the group dir appears here.
      const firstError = await writeTextFenced(fs, target, JSON.stringify(roster, null, 2) + '\n', cwd, ownerId)
      const notes = firstError === undefined ? [] : ['名册写入经沙箱升级完成（首次拒绝：' + firstError + '）']
      return { ok: true, created: true, dir, notes }
    } catch (error) {
      return { ok: false, error: '创建群聊目录失败：' + errMsg(error) }
    }
  }
