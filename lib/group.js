 ...
 */
export const MEMBER_TOOL_DENY = ['group_pull', 'group_create', 'agrp_pull', 'list_agents', 'interrupt_agent', 'ask_user_question']
/** 名字解析不出来时的保守黑名单。 */
...
export function createGroupService(ctx, options) {
  ...
  async function state() {
    ...
    for (const id of Object.keys(registry.groups)) {
      ...
      groups.push({
        ...
        owner_live: liveAgent(id) !== undefined,
  ...
  async function createGroup(args) {
    ...
    if (engine === undefined || typeof engine.ensureSession !== 'function') {
      return { ok: false, error: 'sessionController 不可用，无法创建群聊会话' }
    }
    const id = GROUP_ID_PREFIX + randomUUID()
    try {
      await engine.ensureSession(id, cwd, true, wanted)
    } catch (error) {
    ...
    return { ok: true, id, name: group.name, preset_id: wanted, cwd, capability_warning: capabilityWarning(wanted) }
  }
  ...
  async function ensureGroupAgent(group) {
    ...
    const engine = controller()
    if (engine === undefined || typeof engine.ensureSession !== 'function') {
      throw new Error('group session is not live and sessionController is unavailable')
    }
    await engine.ensureSession(group.id, group.cwd, true, group.presetId)
    agent = liveAgent(group.id)
    if (agent === undefined) throw new Error('group session did not come online')
    return agent
  }
  ...
  async function pull(args) {
    ...
    try {
      ...
      return { ok: false, error: '群主会话不在线，且无法启动它：' + errMsg(error) }
    }
