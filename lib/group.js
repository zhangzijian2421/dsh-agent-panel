...
export function createGroupService(ctx, options) {
   ...
   * 建群 = 启动它的 agent（沿用会话自己的 preset，避免冲突）→ 按需用原生的
   * 空白会话切预设（`agentPresets.select`，非空白会抛 locked）→ 注册进面板 →
   * 标题改成 `👥 <名字>`，让它在侧边栏里**区别于普通会话**。
   *
   * 只允许对空会话使用：会话一旦开始，`select` 会拒绝（locked），此时保留
   * 原 preset 并如实告知，不会让会话处于半途状态。
   * @param args - { session_id, preset_id?, name? }
   */
  async function createGroup(args) {
    ...
    const registry = readFile()
    const name = nonEmpty(args && args.name) || nextGroupName(registry.groups)
    const group = {
      id: sessionId,
      name,
      cwd,
  ...
  return { state, createGroup, renameGroup, pull, release, restore, dissolve, repairTitles, members, file }
}
