...
export function createGroupService(ctx, options) {
  ...
  async function state() {
    ...
    for (const id of Object.keys(registry.groups)) {
      ...
      const workspace = workspaceOf(group, workspaceEntities)
      ...
      groups.push({
        ...
        workspace_id: workspace === undefined ? undefined : workspace.id,
        workspace_title: workspace === undefined ? undefined : workspace.title,
   ...
   */
  async function accountToWorkspace(group) {
    const registry = ctx.get('workspaceRegistry')
    if (registry === undefined || typeof registry.list !== 'function') {
      return { ok: false, error: 'workspaceRegistry 不可用，无法归属到工作区' }
    }
    let entities = []
    try { entities = registry.list() } catch (error) { return { ok: false, error: '读取工作区列表失败：' + errMsg(error) } }
    const target = entities.find((item) => item && samePath(String(item.path), String(group.cwd)))
    if (target === undefined) {
      return { ok: false, error: '该目录还不是工作区（先在 GUI 里把它加为工作区）', cwd: group.cwd }
    }
    const attached = Array.isArray(target.sessionIds) && target.sessionIds.map(String).indexOf(String(group.id)) >= 0
    const info = {
      workspace_id: String(target.id),
      workspace_title: target.title === undefined ? '' : String(target.title)
    }
    if (attached) return { ok: true, already: true, ...info }
    if (typeof target.attachSession !== 'function') {
      return { ok: false, error: '该工作区实体不支持 attachSession', ...info }
    }
    try {
      await target.attachSession(String(group.id))
    } catch (error) {
      return { ok: false, error: '归属到工作区失败：' + errMsg(error), ...info }
    }
    return { ok: true, already: false, ...info }
  }

  /** 一个群当前归属的工作区（只读，给 state 用）。 */
  function workspaceOf(group, entities) {
    const target = entities.find((item) => item && samePath(String(item.path), String(group.cwd)))
    if (target === undefined) return undefined
    const attached = Array.isArray(target.sessionIds) && target.sessionIds.map(String).indexOf(String(group.id)) >= 0
    return attached ? { id: String(target.id), title: target.title === undefined ? '' : String(target.title) } : undefined
  }
  ...
  async function createGroup(args) {
    ...
    // 建完就归属到该目录的工作区——否则它不会出现在侧边栏的工作区下面。
    const account = await accountToWorkspace(group)
    return {
      ok: true,
      id,
      name: group.name,
      ...
      started_via: startedVia,
      owner_model: ownerModel,
      workspace_id: account.ok === true ? account.workspace_id : undefined,
      workspace_title: account.ok === true ? account.workspace_title : undefined,
      workspace_error: account.ok === true ? undefined : (account.already === true ? undefined : account.error),
  ...
  async function pull(args) {
    ...
    // 顺手补一次归属：老群（或当初建群时目录还不是工作区）在这里自愈。
    try { await accountToWorkspace(group) } catch (error) { /* 归属失败不影响拉人 */ }
  ...
  /** 把一个已有的群归属到它的工作区（页面上的「挂到侧边栏」按钮走这里）。 */
  async function attach(args) {
    ...
    if (group === undefined) return { ok: false, error: '群聊不存在：' + id }
    const account = await accountToWorkspace(group)
    if (account.ok !== true) return account
    return { ok: true, group_id: id, workspace_id: account.workspace_id, workspace_title: account.workspace_title, already: account.already }
  }

  return { state, createGroup, renameGroup, pull, release, restore, dissolve, attach, members, file }
}
