window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** The pill itself; both seats (header + blank-session dock) share this one button. */
		...
		function PopPanel() {
			...
			const listed = (snap && snap.groups) || [];
			// The session you are looking at is ALWAYS a pull target, even when the host
			// cannot list it (a session with no workspace cwd in its header). The host
			// resolves the cwd itself on pull, so an empty cwd here is still actionable.
			const known = listed.some((item) => item.session_id === currentSessionId);
			const all = listed.length === 0 || known || currentSessionId === "" ? listed : listed.concat([{
				session_id: currentSessionId,
				cwd: "",
				owner_id: currentSessionId,
				owner_name: "主持人",
				owner_live: false,
				has_roster: false,
				multi_group: true,
			...
			const onPull = (presetId) => {
				...
				api.pull({ sessionId: group.session_id, cwd: group.cwd, ownerId: group.owner_id, presetId }).then(
					(result) => {
						...
						else setNote("已拉入：" + String((result && result.name) || presetId) + (result && result.info ? "。" + String(result.info) : ""));
						setBusy("");
						refresh();
					},
			...
			if (snap === null) {
				...
				if (groups.length > 1) {
					body.push(React.createElement("div", { className: "agrp-sect", key: "picks" },
						...
						React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: "6px" } },
							...
							}, (item.session_id === currentSessionId ? "当前 · " : "") + (baseName(item.cwd) || "未列出工作目录") + (item.has_roster ? "（群 " + item.members.length + "）" : "") + " · " + shortId(item.session_id)))
						)
					));
				}
				body.push(React.createElement("div", { className: "agrp-sect", key: "owner" },
					...
					React.createElement("div", { className: "agrp-muted" },
						(group.cwd ? baseName(group.cwd) : "工作目录未知（拉人时按会话自身解析）") + " (" + shortId(group.session_id) + ") " + (group.owner_live ? "· 在线，可拉人" : "· 未启动（拉人会自动启动它）"),
					...
					group.has_roster
						? null
						: React.createElement("div", { className: "agrp-muted", style: { marginTop: "2px" } },
							"该会话还没有群聊目录：直接点下面的「拉入本会话」即可，拉人后会自动为它创建独立群聊（自动变群）。若该会话用「多 Agent 聊天群」预设创建，成员将拥有 group_send 等群工具；否则成员用 send_message 私信汇报。")
				));
		...
		/** Client plugin body: own the stylesheet and seat the pull trigger (header + blank-session dock) plus the dropdown panel. */
		function apply(ctx) {
