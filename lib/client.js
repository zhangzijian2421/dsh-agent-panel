window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const api = {
			...
			init: (args) => rpc("/api/dsh-agent-panel/init", "POST", args),
		...
		function PopPanel() {
			...
			const [fullCap, setFullCap] = React.useState(false);
			const refresh = () => {
				api.state().then(
			...
			const onInitGroup = () => {
				...
				setBusy("init"); setErr(""); setNote(""); setConfirmId("");
				api.init({ sessionId: group.session_id, cwd: group.cwd }).then(
					(result) => {
						if (result && result.ok === false) setErr(String(result.error || "创建群聊失败"));
						else if (result && result.already) setNote("该会话已经是聊天群");
						else setNote("已为该会话创建独立群聊目录：现在拉人将写入它自己的名册与频道");
						if (result && result.notes && result.notes.length) setNote(String(result.notes.join("；")));
						setBusy("");
						refresh();
					},
			...
			const onPull = (presetId) => {
				...
				api.pull({ sessionId: group.session_id, cwd: group.cwd, ownerId: group.owner_id, presetId, fullCapability: fullCap }).then(
					(result) => {
			...
			if (snap === null) {
					...
					body.push(React.createElement("div", { className: "agrp-sect", key: "picks" },
						...
						React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: "6px" } },
							groups.map((item, i) => React.createElement("button", {
								...
								title: String(item.cwd) + " · " + String(item.session_id),
								onClick: () => { setSel(i); setPicked(true); setConfirmId(""); }
							}, (item.session_id === currentSessionId ? "当前 · " : "") + baseName(item.cwd) + (item.has_roster ? "（群 " + item.members.length + "）" : "") + " · " + shortId(item.session_id)))
						)
					));
				}
				body.push(React.createElement("div", { className: "agrp-sect", key: "owner" },
					...
					group.has_roster
						...
						: React.createElement("div", { style: { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" } },
							React.createElement("span", { className: "agrp-muted" }, "该会话还不是聊天群（无独立群目录）"),
							React.createElement("button", {
								className: "agrp-btn",
								disabled: busy !== "",
								title: "为该会话创建独立群目录 .agent-group/groups/<会话id>/（同一工作区可建多个互不干扰的群）。建议该会话用「多 Agent 聊天群」预设创建，成员才有 group_send/group_read 群工具。",
								onClick: onInitGroup
							}, busy === "init" ? "创建中..." : "初始化为群聊")
						)
				));
				...
				body.push(React.createElement("div", { className: "agrp-sect", key: "presets" },
					...
					React.createElement("label", { style: { display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", cursor: "pointer" } },
						React.createElement("input", {
							type: "checkbox",
							checked: fullCap,
							onChange: (event) => setFullCap(!!(event && event.target && event.target.checked))
						}),
						React.createElement("span", null, "以完整能力运行（recompose 成该 preset，获得其真实工具集）")
					),
					fullCap ? React.createElement("div", { className: "agrp-muted" },
						"已开启：子代理拉起后将 recompose 为该 preset，拥有其真实工具。聊天群成员会自动忽略此选项（保持群工具）。") : null,
					presets.length === 0
