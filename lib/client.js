window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DockTrigger(props) {
			...
			claimSession(props);
			// 空白会话专属：preset 选择 + 「＋ 创建群聊」。点击后这个空会话就变成群聊
			// （宿主会启动它的 agent、按需用原生 select 换 preset、改名并注册进面板）。
			...
			React.useEffect(() => {
				...
				api.state().then(
					(value) => { setPresets((value && value.presets) || []); setPresetId((value && value.default_group_preset) || "standard"); },
					() => { setPresets([]); }
				);
			}, [blank === true]);
			...
			const onCreate = () => {
				...
				setBusy("create");
				api.groupCreate({ session_id: sessionId, preset_id: presetId || "standard" }).then(
					(result) => {
						setBusy("");
			...
			rows.push(React.createElement("select", {
		...
		function PopPanel(props) {
			...
			const onCreate = () => {
				run("create", api.groupCreate({ session_id: currentSessionId, preset_id: newPreset || defaultPreset }), (result) => {
				...
				body.push(React.createElement("div", { className: "agrp-sect", key: "pick" },
					...
					currentBlank === false
						...
						: React.createElement(React.Fragment, null,
							groups.length === 0
								? React.createElement("div", { className: "agrp-muted" }, "还没有群聊。选好群主 preset，点「创建群聊」——这个空会话就变成群聊。")
								: React.createElement("div", { className: "agrp-row" },
