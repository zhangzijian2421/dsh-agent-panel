window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 把群会话**归属**到它所在目录的工作区——这是群聊出现在左侧"工作区下面"的唯一条件
		 * （没归属的会话只算"未分组"）。归属必须在宿主侧做：只有 Workspace 实体的
		 * `attachSession()` 能给会话开户，客户端面的 `insertSessionBefore` 只能重排已归属的会话
		 * （对外部会话报 `the session is not accounted`——实机踩过）。所以这里调宿主路由，
		 * 只有"该目录还不是工作区"时才由客户端创建它再重试。
		 */
		async function attachToWorkspace(sessionId, cwd, items) {
			if (sessionId.length === 0) return "（会话 id 缺失，未归属到工作区）";
			let result;
			try {
				result = await api.groupAttach({ group_id: sessionId });
			} catch (error) {
				return "（归属失败：" + String((error && error.message) || error) + "）";
			}
			if (result && result.ok === true) {
				return result.already === true
					? "已在侧边栏工作区「" + String(result.workspace_title) + "」下"
					: "已挂到侧边栏工作区「" + String(result.workspace_title) + "」下";
			}
			const reason = String((result && result.error) || "未知原因");
			if (reason.indexOf("还不是工作区") < 0) return "（" + reason + "）";
			const service = workspaceService;
			if (service === undefined || service === null || typeof service.create !== "function") {
				return "（该目录不是工作区，且 workspaces 服务不可用，无法创建）";
			}
			try {
				await service.create({ path: cwd });
			} catch (error) {
				return "（新建工作区失败：" + String((error && error.message) || error) + "）";
			}
			try {
				const retry = await api.groupAttach({ group_id: sessionId });
				if (retry && retry.ok === true) return "已新建工作区并挂到侧边栏：" + (baseName(cwd) || String(cwd));
				return "（" + String((retry && retry.error) || "重试归属失败") + "）";
			} catch (error) {
				return "（重试归属失败：" + String((error && error.message) || error) + "）";
			}
		}
		...
		function GroupsPage(props) {
			...
			if (snap === null) {
				...
				for (const group of groups) {
					// 归属状态以宿主为准（state 里的 workspace_title），客户端快照只作兜底。
					const attachedTitle = String(group.workspace_title === undefined || group.workspace_title === null ? "" : group.workspace_title);
					const attached = attachedTitle.length > 0 ? { title: attachedTitle } : attachedTo(String(group.id));
